...
select * from tb_produtos where nome like "%Tê%";
select * from tb_produtos where nome = "Tênis";
...
select * from tb_produtos 
	inner join tb_marcas on tb_marcas.id = tb_produtos.marca_id; /*Trazer informações de uma coluna para a outra pelo ID*/
    
select tb_produtos.nome, tb_produtos.preco, tb_marcas.nome from tb_produtos /*Trazer informações apenas de colunas específicas para a outra pelo ID*/
	inner join tb_marcas on tb_marcas.id = tb_produtos.marca_id
    where tb_marcas.nome Like "%Ni%" /*Trazer informações onde possui no nome as letras "Ni"*/
    and tb_produtos.preco <50.00 /*Trazer informações onde possui no nome as letras "Ni" e o que seja menor do que 50 reais*/
    and tb_produtos.nome = "Meias"; /*Trazer informações onde possui no nome as letras "Ni" e o que seja tem como nome "meias"*/
    
    select tb_produtos.nome, tb_produtos.preco, tb_marcas.nome from tb_produtos /*Trazer informações apenas de colunas específicas para a outra pelo ID*/
	inner join tb_marcas on tb_marcas.id = tb_produtos.marca_id
    where tb_produtos.nome = "Meias" /*Trazer informações de tudo que for meias ou tênis*/
